...
func toolCreateSandbox() mcp.Tool {
	return mcp.NewTool(
		"create_sandbox",
		mcp.WithDescription("Create a new sandbox VM. Returns the sandbox details including its ID."),
		mcp.WithString("name",
			mcp.Required(),
			mcp.Description("Unique name for the sandbox (DNS-1123 subdomain format: lowercase alphanumeric and hyphens)"),
		),
		mcp.WithString("image",
			mcp.Description("Base image name (e.g. 'alpine', 'debian'). Defaults to 'alpine' if omitted."),
		),
		mcp.WithNumber("cpu",
