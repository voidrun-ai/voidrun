...
func (s *SandboxService) Snapshot(ctx context.Context, orgID primitive.ObjectID, id string) error {
	...
	if err := runtime.Snapshot(ctx, id, snapshotDir, s.resolveHV(sandbox.Hypervisor)); err != nil {
		return err
	}
...
func (s *SandboxService) Restore(ctx context.Context, orgID primitive.ObjectID, id string) error {
	...
	hvType := s.resolveHV(sandbox.Hypervisor)
	if err := runtime.Restore(ctx, *s.cfg, spec, overlayPath, snapshotDir, hvType); err != nil {
	...
	cleanup := func() {
		...
		if stopErr := runtime.Stop(ctx, id, hvType); stopErr != nil {
